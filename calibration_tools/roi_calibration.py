...
default_whitebalance = df['whitebalance'][0]
default_x = df['x'][0]
default_y = df['y'][0]
default_w = df['w'][0]
default_h = df['h'][0]
...
def onTrack1(val):
    global default_x
    default_x=val
    print('roi x',default_x)
def onTrack2(val):
    global default_w
    default_w=val
    print('roi w',default_w)
def onTrack3(val):
    global default_y
    default_y=val
    print('roi y',default_y)
def onTrack4(val):
    global default_h
    default_h=val
    print('roi h',default_h)
def onTrack4(val):
    global default_h
    default_h=val
    print('roi h',default_h)
...
while True:
    ...
    frame = image[default_y:default_h+1, default_x:default_w+1]
