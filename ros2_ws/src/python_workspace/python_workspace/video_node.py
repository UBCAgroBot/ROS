...
from std_msgs.msg import Header, String, Int64MultiArray
    ...
    def publish_static_image(self):
        ...
        while success:
            if count == interval:
                ...
                logger_msg = Int64MultiArray()
                logger_msg.data = [start_time, end_time]
                image_input.logger = logger_msg



                # publish image and increment whatever is needed
                self.input_image_publisher.publish(image_input)
                self.get_logger().info(f"Published image {self.image_counter} time: {(end_time-start_time)/1e6} ms")
